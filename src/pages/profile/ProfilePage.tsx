  ...
  return (
    <div className="max-w-6xl mx-auto pb-12 p-4 sm:p-6">
      ...
      <div className="flex items-center gap-4 mb-8">
        <Link 
          to="/dashboard" 
          className="w-12 h-12 flex items-center justify-center rounded-2xl bg-white dark:bg-slate-800 text-slate-500 hover:text-primary dark:hover:text-primary shadow-sm border border-slate-200/60 dark:border-slate-700/60 hover:shadow-md transition-all cursor-pointer"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-white dark:bg-slate-800 rounded-[2rem] p-1.5 shadow-sm border border-slate-200/60 dark:border-slate-700/60 overflow-hidden relative group">
            {/* Cover Banner */}
            <div className="h-36 bg-gradient-to-br from-primary via-primary-dark to-slate-800 rounded-t-[1.75rem] relative overflow-hidden">
            ...
            <div className="px-6 pb-6 flex flex-col items-center text-center relative -mt-16">
              <div className="w-32 h-32 rounded-full bg-white dark:bg-slate-800 p-2 shadow-2xl mb-5 relative z-10">
                <div className="w-full h-full rounded-full bg-primary/10 dark:bg-slate-700/50 flex items-center justify-center text-primary dark:text-primary text-5xl font-bold">
                  {user?.name?.charAt(0)?.toUpperCase() || <FiUser />}
                </div>
              </div>
              
              <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">
                {renderName(user?.name) || 'مستخدم'}
              </h2>
              ...
              {user?.email && (
                <div className="w-full bg-slate-50 dark:bg-slate-700/30 rounded-2xl p-4 mb-6 flex items-center gap-4 text-left">
                  <div className="w-12 h-12 rounded-xl bg-white dark:bg-slate-700 flex items-center justify-center text-slate-400 shadow-sm shrink-0">
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          {/* Appearance Section */}
          <div className="bg-white dark:bg-slate-800 rounded-[2rem] p-8 shadow-sm border border-slate-200/60 dark:border-slate-700/60 transition-all hover:shadow-md">
            <div className="flex items-center gap-4 mb-8">
              ...
              <button
                ...
                className={`relative overflow-hidden flex flex-col items-center justify-center gap-4 p-6 rounded-2xl border-2 transition-all cursor-pointer ${
                  theme === 'light'
                    ...
                    : 'border-slate-100 dark:border-slate-700 text-slate-500 hover:border-slate-300 dark:hover:border-slate-600'
                }`}
              >
                ...
                >
                  <div
                    className={`w-16 h-16 rounded-3xl flex items-center justify-center transition-all shadow-sm ${
                      themeColor === c.id ? 'ring-4 ring-offset-4 ring-offset-white dark:ring-offset-slate-800' : 'opacity-80 hover:opacity-100 hover:shadow-md'
                    }`}
                    style={{ 
          ...
          <div className="bg-white dark:bg-slate-800 rounded-[2rem] p-8 shadow-sm border border-slate-200/60 dark:border-slate-700/60 transition-all hover:shadow-md">
            ...
            <div className="flex items-center gap-3 bg-slate-50 dark:bg-slate-700/30 p-2.5 rounded-3xl border border-slate-100 dark:border-slate-700/50">
              {fontSizes.map((fs) => (
                <button
                  key={fs.id}
                  ...
                  className={`flex-1 py-5 rounded-2xl flex flex-col items-center gap-2 transition-all cursor-pointer ${
                    fontSize === fs.id
                      ? 'bg-white dark:bg-slate-800 shadow-md text-primary dark:text-primary border border-slate-100 dark:border-slate-700'
                      : 'text-slate-500 hover:bg-slate-200/50 dark:hover:bg-slate-600/50 border border-transparent'
                  }`}
                >
